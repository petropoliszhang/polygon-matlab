function varargout = pwld_solve_problem(input_arg)
...
% data
%
geofile = input_arg.geofile;

logi_mms  = input_arg.logi_mms;
mms_type  = input_arg.mms_type;
...
%         end
        ...
        [nel,ndof,connectivity,corner_pos,vert,n_edge,edg2poly,edg2vert,edg_perp,i_mat,i_src,curr_ref_lev] =...
            refine_geom_v2(err_i,ref_threshold,curr_ref_lev,nel,connectivity,corner_pos,vert,...
...
%------------------------------------------------
if(input_arg.save_workspace)
     expression = sprintf('save %s',input_arg.workspace_name');
     eval(expression);
end
%------------------------------------------------
