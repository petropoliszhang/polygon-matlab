...
figure(1)
% uniform
str = strcat(basedir,'rand_quad_mms_3_L1_n2_a0.mat');
...
geo = 'rand_poly_mms_3_L1_n2_a0.9.mat';
for k=1:6
    k1=strfind(geo,'_n');
    str = strcat(dir,geo(1:k1+1),int2str(2^k),geo(k1+3:end));
    expression=sprintf('%s %s','load',str);eval(expression)
    ndof_(k)=norm_data(1);
    erro_(k)=norm_data(2);
end
...
% smooth
geo = 'smoo_poly_mms_3_L1_n2_a0.15.mat';
for k=1:6
    k1=strfind(geo,'_n');
    str = strcat(dir,geo(1:k1+1),int2str(2^k),geo(k1+3:end));
    expression=sprintf('%s %s','load',str);eval(expression)
...
geo = 'shes_poly_mms_3_L1_nc1_a0.15.mat';
for k=1:6
    k1=strfind(geo,'_nc');
    str = strcat(dir,geo(1:k1+2),int2str(k),geo(k1+4:end));
    expression=sprintf('%s %s','load',str);eval(expression)
    ndof_(k)=norm_data(1);
    erro_(k)=norm_data(2);
end
