...
As noted in the Introduction, piece-wise linear functions can be beneficially used to handle transitions
between mesh cells of different refinement levels. In the context of discontinuous finite elements, the
analog of \fig{} is given in \fig{}. With standard linear discontinuous finite elements, the cell to
the left (labelled $K$ in \fig{}) would remain  a quadrilateral after refinement of its neighbor. This also
means that the restriction to edge $AB$ of the intensity from that cell, $E_K^{AB}$, would remain a
