...
\cite{amr stuff}. Adaptive Mesh Refinement (AMR) techniques require local error estimates to determine
which cells to refine. When using discontinuous finite elements, it is observed that jumps are
small in regions where the solution is smooth and are significantly larger in regions where
the solution undergoes rapid spatial variations. A heuristic error indicator is, therefore, to
monitor the interfacial jumps. Following \cite{deal,ragusa-wang1,ragusa-wang2}, we define 
the error indicator on cell $K$ as:
\begin{equation} \label{eq:error_indicator}
  \eta_K = \frac{\int_{\partial K} \jmp{E}^2_K}{\|\Phi_K\|_2^2}.   
\end{equation}  

 As noted in the Introduction, one of the advantages of employing piece-wise linear functions
