...
A piece-wise linear discontinuous (PWLD) finite element discretization of the diffusion equation for arbitrary polygonal meshes
is presented. It is based on the standard diffusion form and uses the symmetric interior penalty technique which yields a 
...
Piece-wise linear approximations allow a straightforward implementation of local mesh adaptation by allowing unrefined cells to
...
Polygonal grids have been generated using bounded Voronoi diagrams using vertex locations from 
severely distorted quadrilateral grids; many meshes contain polygons with a high number of vertices 
(e.g., $>6$). 

One of the attractive features of a PWL discretization resides in its handling of locally refined grids.
Cells adjacent to more refined cells do not possess hanging nodes on the common edges with the refined cells
but rather the nature of the coarse cell is changed to that of a polygon with additional sides. I have
fully embedded this characteristic of PWL finite element within an automated adaptive mesh refinement process
and noted that second-order convergence was retained while reducing the number of unknowns for the same 
...
polygonal meshes using a piece-wise linear discontinuous finite element approximation. Subsequent publications
